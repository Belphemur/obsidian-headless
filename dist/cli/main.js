...
const index_js_2 = require("../encryption/index.js");
const encoding_js_1 = require("../utils/encoding.js");
const engine_js_1 = require("../sync/engine.js");
const lock_js_1 = require("../sync/lock.js");
const engine_js_2 = require("../publish/engine.js");
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const pkgPath = node_path_1.default.join(node_path_1.default.dirname(process.argv[1] || "."), "..", "package.json");
...
function requireAuth() {
    const token = (0, index_js_1.getAuthToken)();
    if (!token) {
        console.error(`No account logged in. Run "${PROGRAM_NAME} login" first.`);
        process.exit(2);
    }
    return token;
}
 ...
 */
function promptInput(message, showInput = false) {
 ...
 */
function checkBtimeModule() {
    if (process.platform !== "win32" && process.platform !== "darwin") {
        return;
    }
    try {
        // Attempt to load the native btime module
        const btimePath = node_path_1.default.join(node_path_1.default.dirname(process.argv[1] || "."), "..", "btime");
        require(btimePath);
    }
    catch {
        console.warn("Warning: Native btime module not loaded. File birth-time tracking may be less accurate.");
    }
}
/**
 ...
 */
function printSyncConfig(config) {
    const modeLabels = {
        "": "bidirectional",
        pull: "pull-only",
        mirror: "mirror-remote",
    };
    const mode = modeLabels[config.syncMode ?? ""] ?? config.syncMode ?? "bidirectional";
    const conflict = config.conflictStrategy ?? "merge";
    const device = config.deviceName ?? (0, index_js_1.getDefaultDeviceName)();
 ...
 */
function printPublishConfig(config) {
...
program
    ...
    .action(async (opts) => {
    const existingToken = (0, index_js_1.getAuthToken)();
    // If already logged in and no args, show current user info
    if (existingToken && !opts.email && !opts.password) {
        try {
            const user = await (0, client_js_1.getUserInfo)(existingToken);
    ...
    if (existingToken) {
        try {
            await (0, client_js_1.signOut)(existingToken);
        }
        catch {
            // Ignore sign-out errors
        }
        (0, index_js_1.clearAuthToken)();
    }
    const email = opts.email ?? (await promptInput("Email: ", true));
    const password = opts.password ?? (await promptInput("Password: "));
    let mfa = opts.mfa;
    try {
        const result = await (0, client_js_1.signIn)(email, password, mfa);
        (0, index_js_1.saveAuthToken)(result.token);
        console.log("Login successful.");
    }
    catch (err) {
        if (err instanceof client_js_1.ApiError && err.message.includes("2FA")) {
            // Need 2FA code
            if (!mfa) {
            ...
            try {
                ...
                console.error(`Login failed: ${err2 instanceof Error ? err2.message : String(err2)}`);
                process.exit(1);
            }
        }
        else {
...
program
    ...
    if (token) {
        try {
            await (0, client_js_1.signOut)(token);
        }
        catch {
...
program
    ...
    const response = await (0, client_js_1.listVaults)(token, SUPPORTED_ENCRYPTION_VERSION);
...
program
    ...
    .action(async () => {
    const vaultIds = (0, index_js_1.listLocalVaults)();
    ...
    for (const id of vaultIds) {
        const config = (0, index_js_1.readSyncConfig)(id);
        if (config) {
            console.log(`  ${config.vaultPath}  (${config.host ?? "default"})`);
        }
    }
});
/**
...
program
    ...
    // Validate region
    if (opts.region) {
        const regionsResponse = await (0, client_js_1.getRegions)(token);
        const regions = regionsResponse
            .regions;
        if (regions) {
            const valid = regions.some((r) => r.id === opts.region || r.name === opts.region);
            if (!valid) {
                console.error(`Invalid region "${opts.region}". Available: ${regions.map((r) => r.id).join(", ")}`);
                process.exit(1);
            }
        }
    }
    let keyHash = "";
    let salt = "";
    const encVersion = opts.encryption === "standard" ? 0 : SUPPORTED_ENCRYPTION_VERSION;
    if (opts.encryption !== "standard" && password) {
        // Generate random 16-byte salt
        salt = (0, encoding_js_1.bufferToHex)((0, encoding_js_1.toArrayBuffer)(node_crypto_1.default.randomBytes(16)));
        const key = await (0, index_js_2.deriveKey)(password, salt);
        keyHash = await (0, index_js_2.computeKeyHash)(key, salt, encVersion);
    }
    const vaultResponse = await (0, client_js_1.createVault)(token, opts.name, keyHash, salt, opts.region ?? "", encVersion);
    console.log(`Vault created successfully.`);
    console.log(`  ID: ${vaultResponse.id ?? "unknown"}`);
...
program
    ...
    .action(async (opts) => {
    const token = requireAuth();
    const vaultsResponse = await (0, client_js_1.listVaults)(token, SUPPORTED_ENCRYPTION_VERSION);
    const allVaults = [
        ...(vaultsResponse.vaults ?? []),
        ...(vaultsResponse.shared ?? []),
    ];
    ...
    if (!vault) {
        ...
        if (byName.length > 1) {
            console.error(`Ambiguous vault name "${opts.vault}". Multiple vaults found. Use the vault ID instead.`);
    ...
    const localPath = node_path_1.default.resolve(opts.path);
    let encKey = "";
    let keySalt = "";
    const encVersion = vault.encryption_version ?? 0;
    // Handle encryption
    if (encVersion > 0) {
        const password = opts.password ?? (await promptInput("Encryption password: "));
        if (!vault.salt) {
            console.error("Vault is encrypted but has no salt. Cannot proceed.");
            process.exit(1);
        }
        keySalt = vault.salt;
        const key = await (0, index_js_2.deriveKey)(password, vault.salt);
        const keyHash = await (0, index_js_2.computeKeyHash)(key, vault.salt, encVersion);
        // Validate access using the derived key
        try {
            await (0, client_js_1.validateAccess)(token, vault.uid ?? vault.id, keyHash, vault.host, encVersion);
        }
        catch {
            console.error("Invalid encryption password.");
            process.exit(1);
        }
        encKey = (0, encoding_js_1.bufferToBase64)(key);
    }
    const deviceName = opts.deviceName ?? (0, index_js_1.getDefaultDeviceName)();
    (0, index_js_1.ensureSyncDir)(vault.id);
    const config = {
        vaultId: vault.id,
    ...
    (0, index_js_1.writeSyncConfig)(vault.id, config);
    ...
    if (node_fs_1.default.existsSync(localPath)) {
        const entries = node_fs_1.default.readdirSync(localPath);
        if (entries.length > 0) {
...
program
    ...
    .action(async (opts) => {
    const localPath = node_path_1.default.resolve(opts.path);
    const config = (0, index_js_1.findSyncConfigByPath)(localPath);
    if (!config) {
        console.error(`No sync configuration found for path "${localPath}". Run "${PROGRAM_NAME} sync-setup" first.`);
        process.exit(1);
    }
    let changed = false;
    ...
    if (opts.excludedFolders !== undefined) {
        config.ignoreFolders = opts.excludedFolders
            .split(",")
            .map((f) => f.trim())
            .filter(Boolean);
        changed = true;
    ...
    if (opts.fileTypes !== undefined) {
        config.allowTypes = (0, index_js_1.parseFileTypes)(opts.fileTypes);
        changed = true;
    }
    if (opts.configs !== undefined) {
        config.allowSpecialFiles = (0, index_js_1.parseConfigCategories)(opts.configs);
        changed = true;
    }
    ...
    if (opts.mode !== undefined) {
        ...
        config.syncMode = modeMap[opts.mode] ?? opts.mode;
        changed = true;
    }
    if (opts.configDir !== undefined) {
        (0, index_js_1.validateConfigDir)(opts.configDir);
    ...
    (0, index_js_1.writeSyncConfig)(config.vaultId, config);
...
program
    ...
    const config = (0, index_js_1.findSyncConfigByPath)(localPath);
    if (!config) {
        console.error(`No sync configuration found for path "${localPath}".`);
        process.exit(1);
    }
...
program
    ...
    (0, index_js_1.removeSyncConfig)(config.vaultId);
...
program
    ...
    const encryption = encVersion > 0 && config.encryptionKey
        ? await (0, index_js_2.createEncryptionProvider)(encVersion, (0, encoding_js_1.base64ToBuffer)(config.encryptionKey), config.encryptionSalt)
        : await (0, index_js_2.createEncryptionProvider)(0, new ArrayBuffer(0), "");
    // Setup log file
    const logPath = (0, index_js_1.getLogPath)(config.vaultId);
    (0, index_js_1.setupLogFile)(logPath);
    // Check btime module availability
    checkBtimeModule();
    // Acquire file lock
    const lockPath = (0, index_js_1.getLockPath)(config.vaultId);
    const lock = new lock_js_1.FileLock(node_fs_1.default, node_path_1.default, lockPath);
    try {
        lock.acquire();
    }
    catch (err) {
        if (err instanceof lock_js_1.LockError) {
            console.error("Another sync instance is already running for this vault.");
            process.exit(1);
        }
        throw err;
    }
...
program
    ...
    .action(async () => {
    const token = requireAuth();
    const response = await (0, client_js_1.listPublishSites)(token);
    ...
    const slugsResponse = await (0, client_js_1.getPublishSlugs)(token, siteIds);
    const slugMap = slugsResponse;
    console.log("Publish sites:");
    for (const site of allSites) {
        const slug = slugMap[site.id] ?? site.slug;
...
program
    ...
    .action(async (opts) => {
    const token = requireAuth();
    const siteResponse = await (0, client_js_1.createPublishSite)(token);
    const siteId = siteResponse.id;
    const siteHost = siteResponse.host;
    if (siteId && siteHost) {
        await (0, client_js_1.setPublishSlug)(token, siteId, siteHost, opts.slug);
    }
...
program
    ...
    (0, index_js_1.writePublishConfig)(site.id, config);
    console.log(`Publish configured for site "${site.slug ?? site.id}".`);
    console.log(`  Path: ${localPath}`);
});
/**
...
program
    ...
    const token = requireAuth();
    const localPath = node_path_1.default.resolve(opts.path);
    const config = (0, index_js_1.findPublishConfigByPath)(localPath);
    if (!config) {
        console.error(`No publish configuration found for path "${localPath}". Run "${PROGRAM_NAME} publish-setup" first.`);
        process.exit(1);
    }
    const engine = new engine_js_2.PublishEngine(token, config);
    ...
    if (!opts.yes) {
        const answer = await promptInput(`Publish ${changes.length} change(s)? [y/N] `, true);
    ...
    await engine.publish(changes, (progress, total) => {
...
program
    ...
    (0, index_js_1.writePublishConfig)(config.siteId, config);
...
program
    ...
    const config = (0, index_js_1.findPublishConfigByPath)(localPath);
    if (!config) {
        console.error(`No publish configuration found for path "${localPath}".`);
        process.exit(1);
    }
    (0, index_js_1.removePublishConfig)(config.siteId);
