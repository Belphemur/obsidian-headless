...
exports.SyncServerConnection = void 0;
...
class SyncServerConnection {
    ...
    queue = new async_js_1.AsyncQueue();
    /** Processes server push notifications. */
    notifyQueue = new async_js_1.AsyncQueue();
     ...
     */
    async connect(url, token, vaultId, version, initial, deviceName, onReady, onPush) {
        if (this.isConnected())
            return;
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.onmessage = (event) => {
                ...
                if (data.status === "err" || data.res === "err") {
                    reject(new Error(data.msg || data.message || "Authentication failed."));
                    return;
                }
     ...
     */
    async push(path, relatedPath, isFolder, isDeleted, ctime, mtime, hash, data) {
        return this.queue.queue(async () => {
            const encodedPath = await this.encryptionProvider.deterministicEncodeStr(path);
            const encodedRelatedPath = relatedPath
                ? await this.encryptionProvider.deterministicEncodeStr(relatedPath)
                : undefined;
            const ext = (0, paths_js_1.extname)((0, paths_js_1.basename)(path));
            ...
            const encryptedHash = await this.encryptionProvider.deterministicEncodeStr(hash);
    ...
    async listDeleted() {
        return this.queue.queue(async () => {
            ...
            for (const item of items) {
                item.path = await this.encryptionProvider.deterministicDecodeStr(item.path);
            }
            return items;
        });
    }
     ...
     */
    async listHistory(path, lastUid) {
        return this.queue.queue(async () => {
            const encodedPath = await this.encryptionProvider.deterministicEncodeStr(path);
            const res = await this.request({
                op: "history",
            ...
            for (const item of items) {
                ...
                if (item.relatedpath) {
                    item.relatedpath =
                        await this.encryptionProvider.deterministicDecodeStr(item.relatedpath);
                }
            }
            return { items };
        });
    ...
    onServerPush(msg) {
        ...
        this.notifyQueue.queue(async () => {
            file.path = await this.encryptionProvider.deterministicDecodeStr(file.path);
            if (file.hash) {
                file.hash = await this.encryptionProvider.deterministicDecodeStr(file.hash);
            }
            if (this.onPush) {
                this.onPush(file);
            }
     ...
     */
    response() {
        this.responsePromise = (0, async_js_1.createDeferred)();
    ...
    dataResponse() {
        ...
        this.dataPromise = (0, async_js_1.createDeferred)();
     ...
     */
    async request(msg, timeout = 60_000) {
...
exports.SyncServerConnection = SyncServerConnection;
