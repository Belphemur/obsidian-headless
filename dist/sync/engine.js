...
const state_store_js_1 = require("../storage/state-store.js");
const adapter_js_1 = require("../fs/adapter.js");
...
class SyncEngine {
    ...
    backoff = new backoff_js_1.ExponentialBackoff(0, 5 * 60 * 1000, 5000, true);
    ...
    constructor(options) {
        ...
        this.stateStore = new state_store_js_1.StateStore((0, index_js_1.getStatePath)(config.vaultId));
        ...
        this.adapter = new adapter_js_1.FileSystemAdapter(node_fs_1.default, node_path_1.default, node_url_1.default, () => false, // no-op trash function
        null, // no native btime setter
        "file:///", config.vaultPath);
        // Initialize sync filter
        this.filter = new filter_js_1.SyncFilter(configDir);
        this.filter.init(config.allowTypes, config.allowSpecialFiles, config.ignoreFolders);
    ...
    handlePush(pushFile) {
        ...
        if (pushFile.wasJustPushed) {
            ...
            this.newServerFiles = this.newServerFiles.filter((f) => f.path !== record.path);
    ...
    async requestSync() {
        ...
        try {
            ...
            while (hasMore) {
                ...
                if (hasMore) {
                    await (0, async_js_1.sleep)(SYNC_RATE_LIMIT_MS);
                }
            }
        }
        finally {
     ...
     */
    canSyncPath(timestamp, filePath) {
     ...
     */
    canSyncLocalFile(timestamp, record) {
        if (record.folder)
            return true;
        let debounceMs;
    ...
    async getServer() {
        ...
        if (!this.server) {
            const conn = new connection_js_1.SyncServerConnection(this.encryption);
            if (!conn.isConnected()) {
                const protocol = this.host.includes("127.0.0.1") ? "ws://" : "wss://";
                const wsUrl = `${protocol}${this.host}`;
                try {
                    await conn.connect(wsUrl, this.token, this.vaultId, this.version, this.initial, this.deviceName, 
                    // onReady callback
                    (serverVersion) => {
                        this.ready = true;
                        if (this.initial) {
    ...
    async _sync() {
        ...
        for (let i = 0; i < this.newServerFiles.length; i++) {
             ...
             */
            const accept = (silent, skipServerUpdate) => {
            ...
            // Validate filename
            const name = (0, paths_js_1.basename)(filePath);
            if (!record.folder && !(0, paths_js_1.isValidFilename)(name)) {
            ...
            if (localFile && !localFile.folder && !localFile.hash) {
                try {
                    ...
                    localFile.hash = await (0, crypto_js_1.sha256Hex)(data);
            ...
            const laterDuplicate = this.newServerFiles.findIndex((f, idx) => idx > i && f.path === filePath);
            if (laterDuplicate !== -1) {
                accept(true, true);
                continue;
            }
            ...
            const ext = (0, paths_js_1.extname)(filePath);
            if (ext === "md" && !record.initial) {
                ...
                try {
                    ...
                    if (remoteData) {
                        remoteContent = (0, encoding_js_1.bufferToString)(remoteData);
                    }
                }
                catch {
                ...
                if (baseRecord && baseRecord.uid) {
                    ...
                    try {
                        ...
                        if (baseData) {
                            baseContent = (0, encoding_js_1.bufferToString)(baseData);
                        }
                    }
                    catch {
                ...
                if (remoteContent !== null) {
                    const merged = (0, merge_js_1.threeWayMerge)(baseContent, localContent, remoteContent);
                    const mergedBuffer = (0, encoding_js_1.stringToBuffer)(merged);
                    const mergedHash = await (0, crypto_js_1.sha256Hex)(mergedBuffer);
            ...
            if (ext === "json" && filePath.startsWith(configDir + "/")) {
                try {
                    ...
                    const remoteData = await server.pull(record.uid);
                    if (remoteData) {
                        const remoteContent = (0, encoding_js_1.bufferToString)(remoteData);
                        ...
                        const mergedBuffer = (0, encoding_js_1.stringToBuffer)(mergedStr);
                        const mergedHash = await (0, crypto_js_1.sha256Hex)(mergedBuffer);
        ...
        for (const [filePath, serverRecord] of Object.entries(this.serverFiles)) {
            if (serverRecord.deleted)
                continue;
            if (!this.filter.allowSyncFile(filePath, serverRecord.folder))
                continue;
            // If file existed locally but no longer does, push deletion
            if (!this.localFiles[filePath]) {
                this.syncingPath = filePath;
                try {
                    await server.push(filePath, null, serverRecord.folder, true, // deleted
                    serverRecord.ctime, Date.now(), "", null);
                    delete this.serverFiles[filePath];
                    this.stateStore.deleteServerFileRecord(filePath);
        ...
        const toUpload = [];
        for (const [filePath, localRecord] of Object.entries(this.localFiles)) {
            if (!this.filter.allowSyncFile(filePath, localRecord.folder))
                continue;
            if (!this.canSyncPath(timestamp, filePath))
                continue;
            ...
            if (!localRecord.folder && localRecord.size > server.perFileMax) {
                this.skippedFiles[filePath] = `exceeds size limit (${(0, format_js_1.formatBytes)(localRecord.size)} > ${(0, format_js_1.formatBytes)(server.perFileMax)})`;
                continue;
            }
            const serverRecord = this.serverFiles[filePath];
        ...
        for (const record of toUpload) {
            ...
            try {
                ...
                if (!record.folder && !record.hash) {
                    const data = await this.adapter.readBinary(filePath);
                    record.hash = await (0, crypto_js_1.sha256Hex)(data);
                    this.stateStore.setLocalFileRecord(record);
                }
                ...
                // Push to server
                await server.push(filePath, null, record.folder, false, // not deleted
                record.ctime, record.mtime, record.hash || "", data);
                // Update server state
                const serverRecord = {
                    path: filePath,
                    hash: record.hash || "",
                ...
                this.log(`[sync] uploaded: ${filePath} (${(0, format_js_1.formatBytes)(record.size)})`);
            }
            catch (err) {
                this.failedSync(filePath);
            }
     ...
     */
    onChange(event, filePath, oldPath, stat) {
        // Skip files we're currently syncing (to avoid feedback loops)
        if (filePath === this.syncingPath)
            return;
        const configDir = this.filter.configDir;
        switch (event) {
            case "file-created": {
                if (!this.filter.allowSyncFile(filePath, false))
                    return;
                const record = {
                    path: filePath,
                    hash: "",
            ...
            case "folder-created": {
                if (!this.filter.allowSyncFile(filePath, true))
                    return;
                const record = {
                    path: filePath,
                    hash: "",
     ...
     */
    async syncFileDown(server, record) {
        const filePath = record.path;
        this.syncingPath = filePath;
        try {
            if (record.folder) {
                ...
                const dir = (0, paths_js_1.dirname)(filePath);
                if (dir && dir !== ".") {
                    await this.adapter.mkdir(dir);
                }
                // Compute hash of downloaded content
                const hash = await (0, crypto_js_1.sha256Hex)(data);
                ...
                this.localFiles[filePath] = localRecord;
                this.stateStore.setLocalFileRecord(localRecord);
                this.log(`[download] ${filePath} (${(0, format_js_1.formatBytes)(data.byteLength)})`);
            }
        }
        finally {
            this.syncingPath = "";
     ...
     */
    async scanConfigDirectory(configDir) {
        ...
        try {
            ...
            for (const file of rootContents.files) {
                if ((0, paths_js_1.extname)(file) === "json") {
            ...
            if (await this.adapter.exists(themesDir)) {
                ...
                for (const themeFolder of themes.folders) {
                    ...
                    for (const file of themeContents.files) {
                        const name = (0, paths_js_1.basename)(file);
            ...
            if (await this.adapter.exists(snippetsDir)) {
                ...
                for (const file of snippets.files) {
                    if ((0, paths_js_1.extname)(file) === "css") {
                        this.scanSpecialFileQueue.push(file);
                    }
                }
            }
