...
class DiffMatchPatch {
            ...
            switch (diffs[pointer][0]) {
                ...
                case DIFF_EQUAL:
                    if (countDelete >= 1 && countInsert >= 1) {
                        ...
                        const subDiffs = this.diff_main(textDelete, textInsert, false, deadline);
                        diffs.splice(pointer - countDelete - countInsert, countDelete + countInsert, ...subDiffs);
    ...
    diff_commonSuffix(text1, text2) {
        ...
        while (lo < mid) {
            if (text1.substring(end1 - mid, end1 - lo) ===
                ...
                case DIFF_EQUAL:
                    if (countDelete + countInsert > 1) {
                        ...
                        if (countDelete === 0) {
                            ...
                            diffs.splice(pointer - count, count, [DIFF_DELETE, textDelete], [DIFF_INSERT, textInsert]);
                        }
    ...
    match_bitap_(text, pattern, loc) {
        ...
        if (best_loc !== -1) {
            score_threshold = Math.min(this.match_bitapScore_(0, best_loc, loc, pattern), score_threshold);
            // Reverse check
        ...
        for (let d = 0; d < pattern.length; d++) {
            ...
            while (start < finish) {
                ...
                if (this.match_bitapScore_(d + 1, loc + mid, loc, pattern) <=
                    score_threshold) {
                    start = mid + 1;
                }
                else {
            ...
            if (this.match_bitapScore_(d + 1, loc, loc, pattern) > score_threshold) {
                break;
            }
            last_rd = rd;
        }
     ...
     * @internal
     */
    match_bitapScore_(e, x, loc, pattern) {
    ...
    patch_addContext(patch, text) {
        ...
        padding += this.patchMargin;
        // Add prefix context
        const prefix = text.substring(Math.max(0, patch.start2 - padding), patch.start2);
        if (prefix) {
    ...
    patch_apply(patches, text) {
        ...
        const results = new Array(patches.length).fill(false);
        let delta = 0;
        for (let x = 0; x < patches.length; x++) {
            ...
            if (text1.length > this.matchMaxBits) {
                ...
                start_loc = this.match_main(text, text1.substring(0, this.matchMaxBits), expected_loc);
                if (start_loc !== -1) {
                    end_loc = this.match_main(text, text1.substring(text1.length - this.matchMaxBits), expected_loc + text1.length - this.matchMaxBits);
                    ...
                    if (text1.length > this.matchMaxBits &&
                        ...
                        for (const diff of patches[x].diffs) {
                            ...
                            if (diff[0] === DIFF_INSERT) {
                                ...
                                const endIndex = this.diff_xIndex(diffs, index1 + diff[1].length);
                                text =
                                    text.substring(0, start_loc + index2) +
